  ...
  <xsl:template match="/">
    <html>
      <head>
        ...
        <script language="JavaScript" src="file:///Users/duncan/Classes/StatComputing/XDynDocs/inst/JavaScript/tooltips.js"/>
      </head>
      <body>
  ...
  <xsl:template name="make-topic-list">
    ...
    <ol>
      <dl>
        ...
        <xsl:for-each select="$nodes[not(not(@status) or @status='done' or @status='high')]">
          <xsl:call-template name="item"/>
        </xsl:for-each>
        <hr width="50%" height="2"/>
        <!-- Done status. -->
