## ---------------------------------------------------------------------------
##
## Hedderik van Rijn, 041105
## hedderik@ai.rug.nl, Artificial Intelligence Groningen
##
##   http://www.ai.rug.nl/~hedderik/R/election2004map
##
## Written for R 2.0.0+ (because of use of transparancy in plotting)
## 
## ---------------------------------------------------------------------------
##
## Code to plot a map of the US elections with, apart from just coloring
## almost each country red, colored circles reflecting the number of voters
## in each county. (I *do* know about the perceptual problems with circles -
## but this way it at least looks like Kerry was close :-)).
##
## Plot is based on the excellent plots found on the New York Times election
## website. 
## 
## ---------------------------------------------------------------------------

## Load map plotting code and databases. Library "maps" is not installed by
## default in R 2+, so, if necessary, install maps first (using something
## like install.packages("maps",lib="~/Library/R/R-2.0.0") and setting
## R_LIBS="$HOME/Library/R" in .Renviron.
library("maps")

## ---------------------------------------------------------------------------

## bluered constructs a color based on the number of votes for B(ush),
## K(erry). If graded==F, color is either blue or red, otherwise color
## ranges from blue to red, reflecting the proportion of votes for either
## candidate. S (originally for Size) is transparancy of the color. Only
## useful when plotting to an OS X quartz device or to
## pdf(...,version="1.4"). Values other than 1 won't show on X11 or other
## devices.
